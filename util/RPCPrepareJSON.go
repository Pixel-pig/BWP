...
func PrepareJSON(method string, params ...interface{}) string {
	rpcRequest := entlity.RPCRequest{
		Id:      time.Now().Unix(),
		Method:  method,
		Jsonrpc: entlity.RPCVERSION,
		Params:params,
	}
	//if params != nil {
...
func DoPost(Body string) (*http.Response, []byte) {
	//1发起一个post请求
	client := &http.Client{}
	request, err := http.NewRequest("POST", entlity.RPCURL, strings.NewReader(string(Body)))
	if err != nil {
		fmt.Println(err.Error())
		return nil, nil
	}
	...
	authStr := entlity.RPCUser + ":" + entlity.RPCPASSWORD
...
//解析数据
func Analysis(response *http.Response, body []byte) *entlity.Result {
	if response.StatusCode == http.StatusOK {
		result := entlity.Result{}
